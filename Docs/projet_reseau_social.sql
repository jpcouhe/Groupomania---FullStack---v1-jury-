...
--
-- Hôte : 127.0.0.1:3306
-- Généré le : mer. 22 juin 2022 à 15:41
...
-- Procédures
--
DROP PROCEDURE IF EXISTS `P_GET_THREADS`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `P_GET_THREADS` (IN `itemsLimit` INT, IN `offsetLimit` INT, IN `userId` INT)  BEGIN
DECLARE itemsLimit2 INT;
DECLARE offsetLimit2 INT;
SET itemsLimit2 = itemsLimit;
SET offsetLimit2 = offsetLimit;
...
WITH tempTable AS (
            ...
            SELECT 
                ...
                SUM(CASE WHEN l.like_user_id = @userId THEN true ELSE false END) isLiked
            FROM tempTable tt
            JOIN thread t
...
GROUP BY t.threads_id, t.title, c.content, c.created_datetime, c.contents_id, u.lastname, u.firstname, u.profile_picture_location,c.users_id
ORDER BY tt.created_datetime DESC
LIMIT itemsLimit2 OFFSET offsetLimit2;
END$$
